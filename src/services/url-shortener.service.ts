...
export const returnShortUrl = async(longUrl: string): Promise<string> => {
  ...
  const shortUrl = `${process.env.BASE_URL}/shorturl/${based64SavedText}`;
  return shortUrl;
};
...
const returnPreviousGeneratedShortUrl = async(longUrl: string): Promise<string|null> => {
  ...
  if (existingUrl) {
    await setRedisValue<UrlShortener>(key, { longUrl, shortUrl: existingUrl.short_url, createdAt: existingUrl.created_at }, 60 * 60 * 24 * 30);
